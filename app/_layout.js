...
// send { type: 'chat', chatId }. A missed-call push (only sent while the
// callee's socket is offline) sends { type: 'call', chatId } - the call
// itself is long over by the time this is tapped, so this just opens the
// chat rather than trying to re-ring.
const handleNotificationTap = (router) => (data) => {
  ...
  } else if ((data.type === 'chat' || data.type === 'call') && data.chatId) {
