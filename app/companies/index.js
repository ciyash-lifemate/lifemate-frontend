        ...
        renderItem={({ item }) => (
          <Pressable style={styles.row} onPress={() => router.push(`/companies/${item.id}`)}>
            ...
            <View style={styles.rowBody}>
              ...
              {item.notes ? <Text style={styles.rowSubtitle} numberOfLines={1}>{item.notes}</Text> : null}
            </View>
            <Ionicons name="chevron-forward" size={20} color={colors.textMuted} />
          </Pressable>
        )}
        ListEmptyComponent={
          !loading ? <Text style={styles.emptyText}>No companies yet. Tap + to add one.</Text> : null
        }
      />
